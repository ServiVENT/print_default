...
const Precio = parametrosURL.get('Precio');
...
function arrayProductos(datos1, datos2, datos3, datos4, datos5, datos6, datos7) {
    let contenido = [datos1, datos2, datos3, datos4, datos5, datos6, datos7];
...
const precioProducto = Precio.split(" , ");
const ImpuestoProducto = Impuesto.split(" , ");
...
const ventas = arrayProductos(cantidadProducto, codigoP, nombreProducto, precioPN, precioIN, precioUN, precioTN);
...
ventas.forEach(p => {
    ...
    fila.innerHTML += `<td class="tdPrecio">${p[3]}</td>`    
    fila.innerHTML += `<td class="tdImpuesto">${p[4]}</td>`
    fila.innerHTML += `<td class="tdPrecioU">${p[5]}</td>`
    fila.innerHTML += `<td class="tdTotal">${p[6]}</td>`
